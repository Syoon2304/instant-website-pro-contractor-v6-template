...
def validate_manifest(
    ...
    if schema_path is None or not schema_path.is_file():
        report.error("manifest.schema_missing", "The protected V6 manifest schema is required.", schema_path or "infrastructure/site-manifest.schema.json")
    else:
        try:
            ...
            if token.startswith(("https://", "http://")):
                ...
                if host is None:
                    ...
                    report.error(
                        ...
                        f"Content-Security-Policy {directive} allows {host}, but the V6 manifest does not declare a feature that needs it.",
                        "_headers",
                    )
        ...
        for _, raw in pattern.findall(text):
            ...
            if not url or url.startswith(("data:", "http://", "https://", "#")):
                if url.startswith("http://"):
                    ...
                    report.error("css.external_asset", "CSS-referenced public assets must be packaged locally under the V6 CSP.", rel)
                continue
            ...
            if tag == "img":
                ...
                for candidate in candidates:
                    if candidate.startswith(("http://", "https://")):
                        report.error("asset.external_image", "Public images must be optimized local files under the V6 CSP.", page.relpath)
...
def validate_icons(
    ...
    for rel, (expected_rel, mime_type, expected_sizes) in required.items():
        ...
        if item.get("rel") != expected_rel or item.get("mime_type") != mime_type or not isinstance(actual_sizes, list) or set(actual_sizes) != expected_sizes:
            report.error("icon.contract", f"Icon declaration does not match the V6 contract: {rel}", "site-manifest.json")
        if not (root / rel).is_file():

...
function buildSubmission(data, request, env) {
  ...
  return {
    submissionId,
    submittedAt,
    siteName: safeLine(env.SITE_NAME || "Contractor website", 120),
...
export async function onRequestPost(context) {
  ...
  if (origin === false) return responseJson({ ok: false, message: "This form must be submitted from the contractor website." }, 403);
  ...
  try {
    ...
    if (!normalizeBoolean(data.privacy_consent)) {
      return responseJson({ ok: false, field: "privacy_consent", message: "Please confirm that the contractor may use this information to respond." }, 400, origin);
    }
